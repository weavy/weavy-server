...
var E = (n, e) => c(n, "name", { value: e, configurable: !0 });
function t(n, e) {
...
function r(n, e, u, $, R) {
  return e.chain = null, e.style = null, e.tail = null, e.tokenize = function(i, p) {
    for (var A = !1, d, g = 0; d = i.next(); ) {
      if (d === u[g] && !A)
        return u[++g] !== void 0 ? (p.chain = u[g], p.style = $, p.tail = R) : R && i.eatWhile(R), p.tokenize = _, $;
      A = !A && d == "\\";
    }
    return $;
  }, e.tokenize(n, e);
...
E(r, "tokenChain");
function O(n, e, u) {
  return e.tokenize = function($, R) {
    return $.string == u && (R.tokenize = _), $.skipToEnd(), "string";
  }, e.tokenize(n, e);
}
E(O, "tokenSOMETHING");
function _(n, e) {
  ...
  if (n.match(/^(\-?((\d[\d_]*)?\.\d+(e[+-]?\d+)?|\d+\.\d*)|0x[\da-fA-F_]+|0b[01_]+|\d[\d_]*(e[+-]?\d+)?)/))
    return "number";
  if (n.match(/^<<(?=[_a-zA-Z])/))
    return n.eatWhile(/\w/), O(n, e, n.current().substr(2));
  ...
  if (u == "/")
    return /~\s*$/.test(I(n)) ? r(n, e, [u], f, l) : "operator";
  if (u == "$") {
    var $ = n.pos;
  ...
  if (/[$@%&]/.test(u) && (n.eatWhile(/[\w$]/) || n.eat("{") && n.eatWhile(/[\w$]/) && n.eat("}"))) {
    var i = n.current();
    return T[i] ? "builtin" : "variable";
  }
  if (u == "#" && t(n, -2) != "$")
    return n.skipToEnd(), "comment";
  if (/[:+\-\^*$&%@=<>!?|\/~\.]/.test(u)) {
    var $ = n.pos;
    if (n.eatWhile(/[:+\-\^*$&%@=<>!?|\/~\.]/), T[n.current()])
      return "operator";
    n.pos = $;
  }
  if (u == "_" && n.pos == 1) {
  ...
  if (/[A-Z]/.test(u)) {
    ...
    if (n.eatWhile(/[A-Z_]/), /[\da-z]/.test(t(n, 0)))
      ...
      var i = T[n.current()];
      return i ? (i[1] && (i = i[0]), p != ":" ? i == 1 ? "keyword" : i == 2 ? "def" : i == 3 ? "atom" : i == 4 ? "operator" : i == 5 ? "builtin" : "meta" : "meta") : "meta";
    }
  }
  if (/[a-zA-Z_]/.test(u)) {
    var p = t(n, -2);
...
const h = {
  ...
  token: /* @__PURE__ */ E(function(n, e) {
    return (e.tokenize || _)(n, e);
  }, "token"),
  languageData: {
    commentTokens: { line: "#" },
    wordChars: "$"
