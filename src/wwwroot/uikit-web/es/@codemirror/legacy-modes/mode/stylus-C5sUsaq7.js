...
a(Q, "escapeRegExp");
var ee = h(B), _ = /^(a|b|i|s|col|em)$/i, ie = h(C), re = h(I), ne = h(S), oe = h(N), te = h(j), ae = Y(j), le = h(W), ce = h(O), de = h(E), se = /^\s*([.]{2,3}|&&|\|\||\*\*|[?!=:]?=|[-+*\/%<>]=?|\?:|\~)/, ue = Y(T), fe = h(U), V = new RegExp(/^\-(moz|ms|o|webkit)-/i), pe = h(F), $ = "", d = {}, b, k, K, t;
function he(i, e) {
  ...
  if (b == '"' || b == "'")
    return i.next(), e.tokenize = X(b), e.tokenize(i, e);
  if (b == "@")
...
a(X, "tokenString");
function ge(i, e) {
  return i.next(), i.match(/\s*[\"\')]/, !1) ? e.tokenize = null : e.tokenize = X(")"), [null, "("];
}
...
a(v, "wordIsBlock");
function R(i) {
  return i.toLowerCase().match(V);
}
a(R, "wordIsVendorPrefix");
function z(i) {
...
const ke = {
  ...
  indent: /* @__PURE__ */ a(function(i, e, r) {
    ...
    return o.prev && (c == "}" && (o.type == "block" || o.type == "atBlock" || o.type == "keyframes") || c == ")" && (o.type == "parens" || o.type == "atBlock_parens") || c == "{" && o.type == "at") ? p = o.indent - r.unit : /(\})/.test(c) || (/@|\$|\d/.test(c) || /^\{/.test(e) || /^\s*\/(\/|\*)/.test(e) || /^\s*\/\*/.test(s) || /^\s*[\w-\.\[\]\'\"]+\s*(\?|:|\+)?=/i.test(e) || /^(\+|-)?[a-z][\w-]*\(/i.test(e) || /^return/.test(e) || v(q) ? p = g : /(\.|#|:|\[|\*|&|>|~|\+|\/)/.test(c) || m(q) ? /\,\s*$/.test(s) ? p = u : /(\.|#|:|\[|\*|&|>|~|\+|\/)/.test(s) || m(s) ? p = g <= u ? u : u + r.unit : p = g : !/,\s*$/.test(e) && (R(q) || y(q)) && (v(s) ? p = g <= u ? u : u + r.unit : /^\{/.test(s) ? p = g <= u ? g : u + r.unit : R(s) || y(s) ? p = g >= u ? u : g : /^(\.|#|:|\[|\*|&|@|\+|\-|>|~|\/)/.test(s) || /=\s*$/.test(s) || m(s) || /^\$[\w-\.\[\]\'\"]/.test(s) ? p = u + r.unit : p = g)), p;
  }, "indent"),
  languageData: {
    indentOnInput: /^\s*\}$/,
