...
var s = (r, c) => L(r, "name", { value: c, configurable: !0 });
function l(r) {
  var c = r.client || {}, g = r.atoms || { false: !0, true: !0, null: !0 }, h = r.builtin || e(C), S = r.keywords || e(m), y = r.operatorChars || /^[*+\-%<>!=&|~^\/]/, o = r.support || {}, v = r.hooks || {}, j = r.dateSQL || { date: !0, time: !0, timestamp: !0 }, N = r.backslashStringEscapes !== !1, Q = r.brackets || /^[\{}\(\)\[\]]/, x = r.punctuation || /^[;.,:]/;
  function b(t, i) {
    var a = t.next();
    if (v[a]) {
      var n = v[a](t, i);
    ...
    if (o.hexNumber && (a == "0" && t.match(/^[xX][0-9a-fA-F]+/) || (a == "x" || a == "X") && t.match(/^'[0-9a-fA-F]*'/)))
      return "number";
    if (o.binaryNumber && ((a == "b" || a == "B") && t.match(/^'[01]+'/) || a == "0" && t.match(/^b[01]*/)))
      return "number";
    if (a.charCodeAt(0) > 47 && a.charCodeAt(0) < 58)
      return t.match(/^[0-9]*(\.[0-9]+)?([eE][-+]?[0-9]+)?/), o.decimallessFloat && t.match(/^\.(?!\.)/), "number";
    if (a == "?" && (t.eatSpace() || t.eol() || t.eat(";")))
      return "macroName";
    if (a == "'" || a == '"' && o.doubleQuote)
      return i.tokenize = w(a), i.tokenize(t, i);
    if ((o.nCharCast && (a == "n" || a == "N") || o.charsetCast && a == "_" && t.match(/[a-z][a-z0-9]*/i)) && (t.peek() == "'" || t.peek() == '"'))
      return "keyword";
    if (o.escapeConstant && (a == "e" || a == "E") && (t.peek() == "'" || t.peek() == '"' && o.doubleQuote))
      return i.tokenize = function(p, q) {
        return (q.tokenize = w(p.next(), !0))(p, q);
      }, "keyword";
    if (o.commentSlashSlash && a == "/" && t.eat("/"))
      return t.skipToEnd(), "comment";
    if (o.commentHash && a == "#" || a == "-" && t.eat("-") && (!o.commentSpaceRequired || t.eat(" ")))
      return t.skipToEnd(), "comment";
    if (a == "/" && t.eat("*"))
      return i.tokenize = f(1), i.tokenize(t, i);
    if (a == ".") {
      ...
      if (y.test(a))
        return t.eatWhile(y), "operator";
      if (Q.test(a))
        return "bracket";
      if (x.test(a))
        return t.eatWhile(x), "punctuation";
      if (a == "{" && (t.match(/^( )*(d|D|t|T|ts|TS)( )*'[^']*'( )*}/) || t.match(/^( )*(d|D|t|T|ts|TS)( )*"[^"]*"( )*}/)))
        return "number";
      t.eatWhile(/^[_\w\d]/);
      var u = t.current().toLowerCase();
      return j.hasOwnProperty(u) && (t.match(/^( )+'[^']*'/) || t.match(/^( )+"[^"]*"/)) ? "number" : g.hasOwnProperty(u) ? "atom" : h.hasOwnProperty(u) ? "type" : S.hasOwnProperty(u) ? "keyword" : c.hasOwnProperty(u) ? "builtin" : null;
    }
  }
  s(b, "tokenBase");
  function w(t, i) {
    return function(a, n) {
      for (var u = !1, p; (p = a.next()) != null; ) {
        if (p == t && !u) {
          n.tokenize = b;
          break;
        }
  ...
  function f(t) {
    return function(i, a) {
      var n = i.match(/^.*?(\/\*|\*\/)/);
      return n ? n[1] == "/*" ? a.tokenize = f(t + 1) : t > 1 ? a.tokenize = f(t - 1) : a.tokenize = b : i.skipToEnd(), "comment";
    };
  }
  s(f, "tokenComment");
  function k(t, i, a) {
  ...
  return s(F, "popContext"), {
    name: "sql",
    startState: /* @__PURE__ */ s(function() {
      return { tokenize: b, context: null };
    }, "startState"),
    token: /* @__PURE__ */ s(function(t, i) {
      if (t.sol() && i.context && i.context.align == null && (i.context.align = !1), i.tokenize == b && t.eatSpace()) return null;
      var a = i.tokenize(t, i);
      if (a == "comment") return a;
      i.context && i.context.align == null && (i.context.align = !0);
      var n = t.current();
      return n == "(" ? k(t, i, ")") : n == "[" ? k(t, i, "]") : i.context && i.context.type == n && F(i), a;
    }, "token"),
    indent: /* @__PURE__ */ s(function(t, i, a) {
      var n = t.context;
      if (!n) return null;
      var u = i.charAt(0) == n.type;
      return n.align ? n.col + (u ? 0 : 1) : n.indent + (u ? 0 : a.unit);
    }, "indent"),
    languageData: {
      commentTokens: {
...
function _(r) {
  for (var c; (c = r.next()) != null; )
    if (c == "`" && !r.eat("`")) return "string.special";
  return r.backUp(r.current().length - 1), r.eatWhile(/\w/) ? "string.special" : null;
}
s(_, "hookIdentifier");
function T(r) {
  for (var c; (c = r.next()) != null; )
...
s(T, "hookIdentifierDoublequote");
function d(r) {
  return r.eat("@") && (r.match("session."), r.match("local."), r.match("global.")), r.eat("'") ? (r.match(/^.*'/), "string.special") : r.eat('"') ? (r.match(/^.*"/), "string.special") : r.eat("`") ? (r.match(/^.*`/), "string.special") : r.match(/^[0-9a-zA-Z$\.\_]+/) ? "string.special" : null;
}
s(d, "hookVar");
function z(r) {
  return r.eat("N") ? "atom" : r.match(/^[a-zA-Z.#!?]/) ? "string.special" : null;
}
s(z, "hookClient");
var m = "alter and as asc between by count create delete desc distinct drop from group having in insert into is join like not on or order select set table union update values where limit ";
function e(r) {
  for (var c = {}, g = r.split(" "), h = 0; h < g.length; ++h) c[g[h]] = !0;
  return c;
}
s(e, "set");
...
l({
  ...
  keywords: e(m + "abort action add after all analyze attach autoincrement before begin cascade case cast check collate column commit conflict constraint cross current_date current_time current_timestamp database default deferrable deferred detach each else end escape except exclusive exists explain fail for foreign full glob if ignore immediate index indexed initially inner instead intersect isnull key left limit match natural no notnull null of offset outer plan pragma primary query raise recursive references regexp reindex release rename replace restrict right rollback row savepoint temp temporary then to transaction trigger unique using vacuum view virtual when with without"),
