...
o(f, "chain");
var _ = /^(?:[-+/%|&^]|\*\*?|[<>]{2})/, z = /^(?:[=!]~|===|<=>|[<>=!]=?|[|&]{2}|~)/, x = /^(?:\[\][?=]?)/, S = /^(?:\.(?:\.{2})?|->|[?:])/, h = /^[a-z_\u009F-\uFFFF][a-zA-Z0-9_\u009F-\uFFFF]*/, k = /^[A-Z_\u009F-\uFFFF][a-zA-Z0-9_\u009F-\uFFFF]*/, A = a([
  "abstract",
  "alias",
  "as",
  "asm",
  ...
  "do",
  "for"
], O = a(s), K = ["if", "unless", "case", "while", "until", "begin", "then"], R = a(K), I = ["end", "else", "elsif", "rescue", "ensure"], D = a(I), b = ["\\)", "\\}", "\\]"], N = new RegExp("^(?:" + b.join("|") + ")$"), y = {
  def: g,
  fun: g,
  macro: Z,
  class: l,
...
function w(n, e) {
  ...
  var r;
  if (n.match(h))
    return n.eat(/[?!]/), r = n.current(), n.eat(":") ? "atom" : e.lastToken == "." ? "property" : A.test(r) ? (O.test(r) ? !(r == "fun" && e.blocks.indexOf("lib") >= 0) && !(r == "def" && e.lastToken == "abstract") && (e.blocks.push(r), e.currentIndent += 1) : (e.lastStyle == "operator" || !e.lastStyle) && R.test(r) ? (e.blocks.push(r), e.currentIndent += 1) : r == "end" && (e.blocks.pop(), e.currentIndent -= 1), y.hasOwnProperty(r) && e.tokenize.push(y[r]), "keyword") : T.test(r) ? "atom" : "variable";
  if (n.eat("@"))
    return n.peek() == "[" ? f(d("[", "]", "meta"), n, e) : (n.eat("@"), n.match(h) || n.match(k), "propertyName");
  if (n.match(k))
    return "tag";
  if (n.eat(":"))
    return n.eat('"') ? f(v('"', "atom", !1), n, e) : n.match(h) || n.match(k) || n.match(_) || n.match(z) || n.match(x) ? "atom" : (n.eat(":"), "operator");
  if (n.eat('"'))
  ...
  return (r = n.match(/^<<-('?)([A-Z]\w*)\1/)) ? f(P(r[2], !r[1]), n, e) : n.eat("'") ? (n.match(/^(?:[^']|\\(?:[befnrtv0'"]|[0-7]{3}|u(?:[0-9a-fA-F]{4}|\{[0-9a-fA-F]{1,6}\})))/), n.eat("'"), "atom") : n.eat("0") ? (n.eat("x") ? n.match(/^[0-9a-fA-F_]+/) : n.eat("o") ? n.match(/^[0-7_]+/) : n.eat("b") && n.match(/^[01_]+/), "number") : n.eat(/^\d/) ? (n.match(/^[\d_]*(?:\.[\d_]+)?(?:[eE][+-]?\d+)?/), "number") : n.match(_) ? (n.eat("="), "operator") : n.match(z) || n.match(S) ? "operator" : (r = n.match(/[({[]/, !1)) ? (r = r[0], f(d(r, F[r], null), n, e)) : n.eat("\\") ? (n.next(), "meta") : (n.next(), null);
}
o(w, "tokenBase");
function d(n, e, r, t) {
  return function(u, i) {
...
o(Z, "tokenMacroDef");
function g(n, e) {
  return n.eatSpace() ? null : (n.match(h) ? n.eat(/[!?]/) : n.match(_) || n.match(z) || n.match(x), e.tokenize.pop(), "def");
}
...
const M = {
  ...
  indent: /* @__PURE__ */ o(function(n, e, r) {
    return e = e.replace(/^\s*(?:\{%)?\s*|\s*(?:%\})?\s*$/g, ""), D.test(e) || N.test(e) ? r.unit * (n.currentIndent - 1) : r.unit * n.currentIndent;
  }, "indent"),
  languageData: {
